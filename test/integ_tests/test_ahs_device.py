...
class TestQnodeIntegration:
    ...
    def test_uploaded_ahs_program_and_pl_result_constants(self):
        """Test that the results of a created AHS program run on the AWS Local simulator roughly
        matches the results of running the same operator in PL, when amplitude, phase and detuning
        are constants."""
        ...
        ahs_local = qml.device("braket.local.ahs", wires=3, shots=10000)
        ...
        H = H_i + qml.pulse.rydberg_drive(amp, phase, det, [0, 1, 2])

        H_obj = qml.PauliZ(0)

        def circuit(t, return_intermediate):
            qml.evolve(H, return_intermediate=return_intermediate)([], t)
            return qml.expval(H_obj)
    ...
    def test_uploaded_ahs_program_and_pl_result_constants(self):
        ...
        amp = qml.pulse.constant
        phase = qml.pulse.constant
        det = qml.pulse.constant
        ...
        def circuit(t, return_intermediate):
            qml.evolve(H, return_intermediate=return_intermediate)([3, 2, 1], t)
            return qml.expval(H_obj)
